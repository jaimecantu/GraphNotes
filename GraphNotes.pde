...
static boolean joining, splitting;
static int index1, index2, index3, index4, count;
...
void setup(){
  ...
  called = false; joining = false; splitting = false;
  exit = false;
  playing = false;
...
void draw(){
  ...
  if(joining){
    for (int i = 0; i<graphs.length;i++){
      if (graphs[i].getSelected()!=null){
        if(aux1 == null && aux2 == null){
          println("aux1 selected");
          aux1 = graphs[i].getSelected();
          graphs[i].deselect();
          index1 = i;
        }
        else if (aux1!=null && aux2==null){
          println("aux2 selected");
          aux2 = graphs[i].getSelected();
          graphs[i].deselect();
          index2 = i;
          aux1.setTarget(aux2);
          joining = false;
